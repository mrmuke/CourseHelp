...
export default function Discover() {
    ...
    return (
        <View>
            <Appbar.Header style={{ backgroundColor: '#008d92', height: 44 }}>
                <Appbar.Content titleStyle={{ fontWeight: 'bold' }} title="Discover" />
            </Appbar.Header>
            <View style={styles.container}>
                <View style={{ backgroundColor: '#008d92', padding: 10, borderRadius: 10, diplay: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                ...
                <ScrollView >
                    {filteredGroups.map((c) => (
                        <Card style={{ margin: 5, marginBottom: 10, backgroundColor: '#eee' }} key={c.id}>
                            <Card.Title style={{ color: '#34646e' }} title={c.name} subtitle={"Surviving " + c.subject} left={props => <Icon name="group" {...props} />} />
                            <Card.Content>
                                <Caption style={{ marginBottom: 10 }}>{c.description}</Caption>
                            </Card.Content>
                            <Card.Actions>
                                {c.members && c.members.some(e => e === firebase.auth().currentUser.uid) ? <Button labelStyle={{ color: "white" }} style={{ flex: 1 }} mode="contained" color="#008d92" onPress={() => leaveGroup(c)}>Leave</Button> : c.publicity === "public" ? <Button labelStyle={{ color: "white" }} onPress={() => joinGroup(c)} style={{ flex: 1 }} mode="contained" color="#008d92">Join</Button> : c.pending && c.pending.some(e => e == firebase.auth().currentUser.uid) ? <Button labelStyle={{ color: "white" }} style={{ flex: 1 }} mode="contained" color="#008d92">Awaiting Approval</Button> : <Button labelStyle={{ color: "white" }} onPress={() => applyGroup(c)} style={{ flex: 1 }} mode="contained" color="#008d92">Apply</Button>}
